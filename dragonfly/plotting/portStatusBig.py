...
def create_subplot(subplot_name, type_plot, y_values, x_values, plot_title, portsInfo):
    ...
    if type_plot == "global":
        subplot_name.bar(x_values - 0.4, y_values[0], width=0.4, edgecolor='black', color=sharedValues.color_global_ports[0] , linewidth=0.0) # offset of -0.4
        subplot_name.bar(x_values, y_values[1], width=0.4, edgecolor='black', color=sharedValues.color_global_ports[1] , linewidth=0.0) # offset of  0.4
        subplot_name.bar(x_values + 0.4, y_values[2], width=0.4, edgecolor='black', color=sharedValues.color_global_ports[2] , linewidth=0.0) # offset of  0.4
    elif type_plot == "host":
        center_label_pos = np.linspace(3.5,224,64)
        sub_pos = np.linspace(0.3,3.5,17)
        sub_pos = sub_pos[:-1]
        sub_pos = sub_pos[::-1]
    ...
    if type_plot == "intra":
        center_label_pos = np.linspace(3.5,224,64)
        sub_pos = np.linspace(0.3,3.5,16)
        sub_pos = sub_pos[:-1]
        sub_pos = sub_pos[::-1]
        for i in range (0, len(y_values)):
            minus = float(sub_pos[i])
...
list_ports_global, list_ports_hosts, list_intra, portsInfo = get_data_from_files("input_for_plots/parse/", "minimal", plot_type)
...
list_ports_global_valiant, list_ports_hosts_valiant, list_intra_valiant, portsInfo = get_data_from_files("input_for_plots/parse/", "valiant", plot_type)
...
plt.suptitle("Shift (no last group) - MSG 10MB", weight = 'bold')
ax_lst = [ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9]

#Minimal
create_subplot(ax_lst[0], "global", list_ports_global, X, "Global Links Port Occupancy per Router", portsInfo)   
create_subplot(ax_lst[1], "host", list_ports_hosts, X, "Host-Router Links Port Occupancy per Router", portsInfo) 
create_subplot(ax_lst[2], "intra", list_intra, X, "Intra-Group Links Port Occupancy per Router", portsInfo) 
...
plt.gcf().set_size_inches(16, 9.5)
...
for idx, subplot in enumerate(ax_lst):
    ...
    if idx == 1 or idx == 4 or idx == 7:
        subplot.axvline(x=55.95 * 1, color='grey', ls=':', alpha=0.5)
        subplot.axvline(x=55.95 * 2, color='grey', ls=':', alpha=0.5)
        subplot.axvline(x=55.95 * 3, color='grey', ls=':', alpha=0.5)
        x_pos_label = []
        x_label_text = []
        group_id = 0
        old = 0
        for i in range (56, 225, 56):
            x_pos_label.append((i + ((i - old) / 2) - 56))
            x_label_text.append("Switches Group {}".format(group_id))
            group_id += 1
            old = i
...
ax3.text(239.5,25.5,"Minimal", size=16,
                           verticalalignment='center', rotation=270)
ax6.text(239.5,25.5,"Valiant", size=16,
                           verticalalignment='center', rotation=270)
ax9.text(239.5,25.5,"UGAL", size=16,
                           verticalalignment='center', rotation=270)
