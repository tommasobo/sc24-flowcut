...
def create_subplot(subplot_name, subplot_title, list_minimal, list_minimal_name, list_data_algo, list_name_algo, color, without_bg, type_plot):
    ...
    violin_parts = subplot_name.violinplot(list_data_algo, showextrema=False, showmedians=True, showmeans=True, points=800, quantiles=list_quantiles)
...
def get_data_from_files(path_dir, is_bg=False, type_plot="latency", file_name="min"):
        ...
        for file in sorted(files):
            ...
            with open(path_dir + file) as fp:
                ...
                for line in Lines:
                    ...
                    if "Using Cached Flowlet\n" == line:
                        num_cached += 1
                    
                    if "New Flowlet Slingshot\n" == line:
                        num_slingshot += 1

                    if "Using Cached Slingshot\n" == line:
    ...
    return percentage_new, percentage_cached, list_algo
...
def main():
    ...
    parser.add_argument('--type', required=False, default="ratio")
    ...
    flow1_new, flow1_cached, flow1_algo = get_data_from_files("input_for_plots/parse/", type_plot=args.type, file_name="flow1")
    flow2_new, flow2_cached, flow2_algo = get_data_from_files("input_for_plots/parse/", type_plot=args.type, file_name="flow2")
    flow3_new, flow3_cached, flow3_algo = get_data_from_files("input_for_plots/parse/", type_plot=args.type, file_name="flow4")
    sling_new, sling_cached, sling_algo = get_data_from_files("input_for_plots/parse/", type_plot=args.type, file_name="sling")
    ...
    new_ratio_list.append(flow1_new)
    new_ratio_list.append(flow2_new)
    new_ratio_list.append(flow3_new)
    new_ratio_list.append(sling_new)

    cached_ratio_list.append(flow1_cached)
    cached_ratio_list.append(flow2_cached)
    cached_ratio_list.append(flow3_cached)
    cached_ratio_list.append(sling_cached)
    ...
    x = np.arange(len(algo_name))

    print(new_ratio_list)
    print(cached_ratio_list)
    print(algo_name)
    rects1 = ax.bar(x - width/2, new_ratio_list, width, color=sharedValues.color_main_traffic, alpha=sharedValues.alpha_bar_plots)
    rects2 = ax.bar(x + width/2, cached_ratio_list, width, color=sharedValues.color_background_traffic, alpha=sharedValues.alpha_bar_plots)
    ...
    ax.set_title(
            label = "Ratio new vs cached path selection running AllToAll motif (32 KiB)",
            weight = 'bold',
            horizontalalignment = 'left',
            x = 0,
            y = 1.045,
    ...
    # ylabel
    ax.set_ylabel(
        "Type of path selection per packet compared to total pacekts seen (%)",
        horizontalalignment='left',
        rotation = 0,
        fontsize=11.4
    )
    ...
    x_pos = np.arange(len(algo_name))
    plt.xticks(x_pos, algo_name)


    # Custom Legend
    if args.without_bg is True:
        legend_dict = {'New path selection' : sharedValues.color_main_traffic, 'Cached path selection' : sharedValues.color_background_traffic}
    ...
    #ax.legend(handles=patchList,bbox_to_anchor=(x_pos, -0.07), ncol=num_col)
    ...
    plt.savefig(Path("plots/png") / (str("ratio_cached") + file_name))
    plt.savefig(Path("plots/pdf") / (str("ratio_cached") + file_name + ".pdf"))
