...
def create_subplot(subplot_name, type_plot, y_values, x_values, plot_title, portsInfo, routing="defa"):
    if plot_title != "":
    ...
    if type_plot == "global":
        ...
        for i in range (0, len(y_values)):
            ...
            if routing == "sling":
                y_values[i][:] = [x+0 for x in y_values[i]]
    ...
    if type_plot == "intra":
        ...
        for i in range (0, len(y_values)):
            minus = float(sub_pos[i])
            '''if routing == "min":
                y_values[i][:] = [0 for x in y_values[i]]'''
...
list_ports_global_ugal, list_ports_hosts_ugal, list_intra_ugal, portsInfo = get_data_from_files("input_for_plots/parse/", "flow2", plot_type)
list_ports_global_valiant, list_ports_hosts_valiant, list_intra_valiant, portsInfo = get_data_from_files("input_for_plots/parse/", "aries", plot_type)
list_ports_global_sling, list_ports_hosts_sling, list_intra_sling, portsInfo = get_data_from_files("input_for_plots/parse/", "sling", plot_type)
...
plt.suptitle("Ports Occupancy Snapshot - Uniform Traffic motif (4 MiB)", weight = 'bold')
...
create_subplot(ax_lst[2], "intra", list_intra, X, "Intra-Group Links Port Occupancy per Switch", portsInfo, "min") 
...
#SLING
create_subplot(ax_lst[9], "global", list_ports_global_sling, X, "", portsInfo, "sling")   
create_subplot(ax_lst[10], "host", list_ports_hosts_sling, X, "", portsInfo, "sling") 
create_subplot(ax_lst[11], "intra", list_intra_sling, X, "", portsInfo, "sling") 
...
for idx, subplot in enumerate(ax_lst):
    ...
    if idx == 0:
        ...
        subplot.legend(handles=patchList, loc='upper right')
    ...
    if (idx <= 2):
        # ylabel
        subplot.set_ylabel(
            "Number of cycles busy per port (%)",
            horizontalalignment='left',
            rotation = 0,
            fontsize=11.6
        )
        subplot.yaxis.set_label_coords(0, 1.005)
    subplot.xaxis.set_tick_params(labelsize=11.6)
    subplot.yaxis.set_tick_params(labelsize=11.6)
    

    # Host Graphs
    if idx == 1 or idx == 4 or idx == 7 or idx == 10:
        width_one_router = 3.2
        subplot.axvline(x=width_one_router * 16 * 1, color='grey', ls=':', alpha=0.65)
        subplot.axvline(x=width_one_router * 16 * 2, color='grey', ls=':', alpha=0.65)
        subplot.axvline(x=width_one_router * 16 * 3, color='grey', ls=':', alpha=0.65)
        half_size = 3.2 * 16 / 2
        full_size = 3.2 * 16
        x_pos_label = [half_size, half_size+full_size, half_size+(full_size * 2), half_size+(full_size * 3)]
        x_label_text = []
        for i in range (1, 5, 1):
            x_label_text.append("Switches\nGroup {}".format(i))
    ...
    if idx == 2 or idx == 5 or idx == 8 or idx == 11:
        subplot.axvline(x=48 * 1, color='grey', ls=':', alpha=0.65)
        subplot.axvline(x=48 * 2, color='grey', ls=':', alpha=0.65)
        subplot.axvline(x=48 * 3, color='grey', ls=':', alpha=0.65)
        half_size = 3 * 16 / 2
        full_size = 3 * 16
        x_pos_label = [half_size, half_size+full_size, half_size+(full_size * 2), half_size+(full_size * 3)]
        x_label_text = []
    ...
    # Global Graphs
    if idx == 0 or idx == 3 or idx == 6 or idx == 9:
...
'''
ax3.text(194.5,46.5,"ECMP", size=17,
                           verticalalignment='center', rotation=270)
ax6.text(194.5,46.5,"UGAL-4", size=17,
                           verticalalignment='center', rotation=270)
ax9.text(194.5,46.5,"Flowlet Balanced", size=14.5,
                           verticalalignment='center', rotation=270)
ax12.text(194.5,46.5,"Slingshot", size=17.0,
                           verticalalignment='center', rotation=270)
