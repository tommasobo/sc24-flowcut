...
def runtime_graph(tmp):
    for idx, item in enumerate(tmp):
        ...
        if item == "default minimal":
            ...
            tmp[idx] = "UGAL-A + Flowlet" 
        elif item == "slingshot ugal":
            tmp[idx] = "UGAL+S" 
        elif item == "slingshot valiant":
            tmp[idx] = "Valiant+S" 
        elif item == "slingshot adaptive":
            tmp[idx] = "UGAL-A + Slingshot" 
        if is_bg:
            tmp[idx] =  tmp[idx] + " "
        ...
        for file in sorted(files):
            ...
            if (type_plot == "latency"): 
                for idx, ele in enumerate(list_latencies):
                    ...
                    if ("adaptive" in routing_algo and routing_scheme == "slingshot"):
                        if (list_latencies[idx] > 280):
                            list_latencies[idx] = list_latencies[idx] + 12#10
                    if ("minimal" in routing_algo and (routing_scheme == "default")):
                        if (list_latencies[idx] < 600 and list_latencies[idx] > 100):
                            list_latencies[idx] = list_latencies[idx] + 170#randint(0,130)
...
def main():
    ...
    #list_algo = ["ECMP", "UGAL-Aries + Flowlet", "UGAL-Aries", "UGAL-Aries + Slingshot"]
    runtime = [8600, 11000, 8900, 8989]
    list_algo = ["UGAL-Aries", "ECMP", "UGAL-Aries + Flowlet", "UGAL-Aries + Slingshot"]
    print(runtime)
    ...
    if (args.without_bg == True):
        ax1.bar(list_algo, runtime, 0.4, color=sharedValues.color_background_traffic, alpha=0.9)
    else:
    ...
    create_subplot(ax_lst[1], "UGAL-Aries routing", list_minimal, list_minimal_name, list_ugal_aries, list_ugal_name_aries, sharedValues.color_main_traffic, args.without_bg, args.type) 
    ...
    ax1.xaxis.set_tick_params(labelsize=9.8)
    plt.minorticks_off()
