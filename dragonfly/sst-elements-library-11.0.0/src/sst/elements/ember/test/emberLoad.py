...
try:
    opts, args = getopt.getopt(sys.argv[1:], "", ["topo=", "shape=","hostsPerRtr=",
                 ...
                 "emberVerbose=","netBW=","netPktSize=", "routing=", "algorithm=", "flowlet_timeout=","ackMode=", "netFlitSize=",
                 ...
                 "useSimpleMemoryModel","param=","paramDir=","statsModule=", 'exponential_alpha=', "statsFile=", 'flowcut_max_ratio=', 'flowcut_mode=', 'flowcut_value='])
...
for o, a in opts:
    if o in ("--shape"):
        ...
        customRouting = a
    elif o in ("--flowlet_timeout"):
        flowletTimeout = a
    elif o in ("--ackMode"):
        ackMode = a
    elif o in ("--flowcut_mode"):
        flowcut_mode = a
    elif o in ("--flowcut_value"):
        flowcut_value = a
    elif o in ("--flowcut_max_ratio"):
        flowcut_max_ratio = a
    elif o in ("--exponential_alpha"):
        exponential_alpha = a
    elif o in ("--algorithm"):
...
if customRouting == "":
    customRouting = "default"

if flowletTimeout == "":
    flowletTimeout = "1000"

if ackMode == "" or ackMode == "False" or ackMode == "false":
    ackMode = "0"
...
if flowcut_value == "":
    flowcut_value = "1000000"

if flowcut_max_ratio == "":
    flowcut_max_ratio = "4"

if exponential_alpha == "":
    exponential_alpha = "0.25"

if ackMode == "True" or ackMode == "true":
    ackMode = "1"

if defaultCustomRouting == "":
    defaultCustomRouting = "minimal"

if customRouting != "flowlet" and customRouting != "default"and customRouting != "slingshot":
    sys.exit("What routing scheme? [default|flowlet|slingshot]")
...
if "torus" == netTopo:
    ...
    if usePlatNetConfig:
        topoInfo = DragonFyInfo(platNetConfig, customRouting, defaultCustomRouting, flowletTimeout, ackMode, flowcut_mode, flowcut_value, flowcut_max_ratio, exponential_alpha)
    else:
        topoInfo = DragonFlyInfo(netShape, customRouting, defaultCustomRouting,flowletTimeout, ackMode, flowcut_mode, flowcut_value, flowcut_max_ratio, exponential_alpha)
    topo = topoDragonFly()
...
if rtrArb:
    print ("EMBER: network: topology={0} shape={1} arbitration={2} routing={3} defaultRouting={4}".format(netTopo,netShape,rtrArb,customRouting, defaultCustomRouting))
else:
    print ("EMBER: network: topology={0} shape={1} routing={2} defaultRouting={3}".format(netTopo,netShape, customRouting, defaultCustomRouting))
...
print("Nic BW " + str(nicParams["link_bw"] ))
...
print("Merlin BW " + str(sst.merlin._params["link_bw"]))
