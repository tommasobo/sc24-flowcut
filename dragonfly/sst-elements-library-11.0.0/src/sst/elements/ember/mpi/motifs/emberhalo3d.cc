	...
	m_loopIndex(0)
{
	...
	iterations = params.find<uint32_t>("arg.iterations", 3);
    ...
    verbose(CALL_INFO, 1, 0, "loop=%d\n", m_loopIndex );
    	...
    	*/
		...
		if(x_down > -1) {
			...
			enQ_irecv( evQ, x_down, items_per_cell * sizeof_cell * ny * nz, TAG, GroupWorld, req);
		}
		...
		if(x_up > -1) {
			...
			enQ_irecv( evQ, x_up, items_per_cell * sizeof_cell * ny * nz, TAG, GroupWorld, req);
		}

		if(x_down > -1) {
			enQ_send( evQ ,x_down, items_per_cell * sizeof_cell * ny * nz, TAG, GroupWorld);
		}

		if(x_up > -1) {
			enQ_send( evQ ,x_up, items_per_cell * sizeof_cell * ny * nz, TAG, GroupWorld);
		}
		...
		if(y_down > -1) {
			...
			enQ_irecv( evQ, y_down, items_per_cell * sizeof_cell * nx * nz, TAG, GroupWorld, req);
		}
		...
		if(y_up > -1) {
			...
			enQ_irecv( evQ, y_up, items_per_cell * sizeof_cell * nx * nz, TAG, GroupWorld, req);
		}

		if(y_down > -1) {
			enQ_send( evQ ,y_down, items_per_cell * sizeof_cell * nx * nz, TAG, GroupWorld);
		}

		if(y_up > -1) {
			enQ_send( evQ ,y_up, items_per_cell * sizeof_cell * nx * nz, TAG, GroupWorld);
		}
		...
		if(z_down > -1) {
			...
			enQ_irecv( evQ, z_down, items_per_cell * sizeof_cell * ny * nx, TAG, GroupWorld, req);
		}
		...
		if(z_up > -1) {
			...
			enQ_irecv( evQ, z_up, items_per_cell * sizeof_cell * ny * nx, TAG, GroupWorld, req);
		}

		if(z_down > -1) {
			enQ_send( evQ ,z_down, items_per_cell * sizeof_cell * ny * nx, TAG, GroupWorld);
		}

		if(z_up > -1) {
			enQ_send( evQ ,z_up, items_per_cell * sizeof_cell * ny * nx, TAG, GroupWorld);
		}
