    ...
    m_getRespSmallVN(0)
{
    ...
    //printf("Send machine %d\n", numSendMachines);
    if ( numSendMachines < 1 ) {
        m_dbg.fatal(CALL_INFO,-1,"Error: numSendMachines must be at least 1, requested %d\n",numSendMachines);
    }
    ++numSendMachines;
    ...
    for ( int i = 0; i < m_numVN; i++ ) {
        m_recvMachine.push_back( new RecvMachine( *this, i, m_vNicV.size(), m_myNodeId,
                ...
                params.find<int>( "maxActiveRecvStreams", 1024 ),
...
Nic::~Nic()
{
    ...
    for (auto& it: list_fct_first_time) {
        key.push_back(it.first);
        //printf("Key is %s\n", it.first.c_str());
    }

    if (packets_received_overall > 100) {
        printf("Overall Bandwidth NIC %d was %d - %f - %ld\n",m_myNodeId, packets_received_overall, (float)packets_received_overall / time_passed, time_passed);
    }

    printf("Total Time in Draining NIC %d is %ld \n", m_myNodeId, time_draining);

    for (int i = 0; i < key.size(); i++) {
        if (list_fct_size[key[i]] > 50) {
            printf("\nFlow BandWidth %f %ld - %ld %ld - %d - %s\n", (float)((float)list_fct_size[key[i]] / (list_fct_last_time[key[i]] - list_fct_first_time[key[i]])) * 8, list_fct_last_time[key[i]] - list_fct_first_time[key[i]], list_fct_first_time[key[i]], list_fct_last_time[key[i]], list_fct_size[key[i]], key[i].c_str()); fflush(stdout);
            //printf("\nFlow %s - Time %ld - BandWidth %f\n", key[i].c_str(), list_fct_last_time[key[i]] - list_fct_first_time[key[i]], (float)list_fct_size[key[i]] / (list_fct_last_time[key[i]] - list_fct_first_time[key[i]])); fflush(stdout);
        }
    }
...
void Nic::feedTheNetwork( int vn )
{
    ...
    while ( ! pq.empty() ) {
        ...
        /*
        // If draining stop sending until matching stop draining packet has been received
        if (this->draining) {
            //ret = false;
            printf("Blocking Packet %d@%d@%d@%d - %ld\n", m_myNodeId, IdToNet(x.dest), x.pkt->msg_id, x.pkt->packet_id_in_message, getCurrentSimTimeNano());
        } else {
            printf("Not Blocking\n"); fflush(stdout);
        }
        */
        ...
        if ( ! ret ) {
            ...
            //if (m_myNodeId == 0)
            //    printf("Nic Send Machine7 %ld - Size %d\n", getCurrentSimTimeNano(), x.pkt->bufSize()); fflush(stdout);
...
{
    assert( ev->bufSize() );
    //printf("Sending Packet2 %d - %d@%d@%d@%d - %ld\n", ev->bufSize(), m_myNodeId, IdToNet( dest ), ev->msg_id, ev->packet_id_in_message, getCurrentSimTimeNano());
    ...
    if (req->size_in_bits / 8 == 8) {
        //printf("Testing Dest %d - Src %d - VN %d \n", req->dest, req->src, req->vn); fflush(stdout);
    }
