    ...
    #include "nicShmemStream.h"
      ...
      public:
            ...
            m_maxPendingPkts(maxPendingPkts)
        {
            ...
            //printf("MAx pending is %d %d %d\n", maxPendingPkts, maxActiveStreams, maxQsize);
        }
	...
	private:
        ...
        void clockHandler() {
            ...
            if ( m_numPendingPkts < m_maxPendingPkts ) {
                ...
                //printf("reached max buffered packets, numPendingPkts=%d\n", m_numPendingPkts );
			...
			while ( iter != m_pktBuf.end() ) {
				...
				if ( ev->isCtrl() ) {
					...
					if ( m_numActiveStreams < m_maxActiveStreams ) {
						...
						--m_numPendingPkts;
						iter->second.pop();
                        //printf("new stream numActiveStreams=%d m_numPendingPkts=%d\n",m_numActiveStreams,m_numPendingPkts-1);
					} else {
                        //printf("can't start new stream numActiveStreams=%d\n",m_numActiveStreams);
					...
					if ( ! m_streamMap[iter->first]->isBlocked( ) ) {
                        ...
                        //printf("stream consumed packet, m_numPendingPkts=%d\n",m_numPendingPkts);
						iter->second.pop();
					} else {
				...
				if ( iter->second.empty() ) {
                    ...
                    //printf("queue is empty clear pktBuf slot\n");
			...
			if ( m_pktBuf.empty() && ! m_nic.m_linkControl->requestToReceive( m_vn )) {
                ...
                //printf("pktBuf is empty, %d %d\n", m_pktBuf.empty(), m_nic.m_linkControl->requestToReceive( m_vn ));
                setNotify();
                m_clocking = false;
			} else {
                ...
                //printf("schedule clockHandler m_numPendingPkts=%d m_numActiveStreams=%d latency=%d -- empty %d, request %d\n",m_numPendingPkts,m_numActiveStreams,m_clockLat, m_pktBuf.empty(), m_nic.m_linkControl->requestToReceive( m_vn ));
            }
        }


        FireflyNetworkEvent* getNetworkEvent(int vn ) {
            //printf("Before Recv Machine\n"); fflush(stdout);
            SST::Interfaces::SimpleNetwork::Request* req = m_nic.m_linkControl->recv(vn);
            //printf("Recv Machine is %d - src %d\n", req->size_in_bits / 8, req->src); fflush(stdout);
