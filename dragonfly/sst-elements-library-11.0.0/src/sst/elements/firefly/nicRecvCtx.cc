...
Nic::RecvMachine::StreamBase* Nic::RecvMachine::Ctx::newStream( FireflyNetworkEvent* ev )
{
    ...
    // Exception for Drain Control Type of Messages
    /*if (ev->is_drain_control) {
        printf("Received a drain control\n"); fflush(stdout);
        return;
    }*/

    printf("Error\n"); fflush(stdout);
    printf("Ev Src %d %d - Ev Dest %d - Ev Size %d\n", ev->getSrcNode(), ev->getSrcPid(), ev->getDestPid(), ev->bufSize()); fflush(stdout);
    assert(0);
}
