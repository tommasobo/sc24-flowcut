...
struct X {
    ...
    int packets_received_overall = 0;
    uint64_t time_passed = 0;
    uint64_t first_packet_time = 0;

    uint64_t time_draining = 0;
    uint64_t latest_drain_start = 0;

    std::unordered_map<std::string,  uint64_t> list_fct_size;

    std::unordered_map<std::string,  uint64_t> list_fct_first_time;
    std::unordered_map<std::string,  uint64_t> list_fct_last_time;

    std::unordered_map<std::string,  bool> draining_map;
