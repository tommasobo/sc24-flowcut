...
public:
    ...
    int getECMPGroup(int dest_group, RNG::SSTRandom* rng) const;
...
public:
    ...
    int num_vcs;
    int num_vns;
    CustomRouting custom_routing_algorithm;
...
public:
    ...
    virtual void set_vc_route(int port, int vc, internal_router_event* ev);
...
private:
    ...
    std::unordered_map<std::string, int> map_cached_gl;
    std::unordered_map<std::string, int> map_cached_duration;
    std::unordered_map<std::string, uint64_t> map_cached_cycle;
    std::unordered_map<std::string, uint32_t> map_cached_mid;
    std::unordered_map<std::string, uint32_t> map_cached_gl_shadow;
    std::unordered_map<std::string, uint32_t> map_cached_mid_shadow;
    std::unordered_map<std::string, bool> map_cached_info;

    int duration_flowlet = 0;
    int duration_slingshot = 0;
