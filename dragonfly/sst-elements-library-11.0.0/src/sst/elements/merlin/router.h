...
public:
    ...
    inline SST::Interfaces::SimpleNetwork::Request* getReq() { return request; }
...
class internal_router_event : public BaseRtrEvent {
    ...
    RtrEvent* encap_ev;
    int64_t sent_time; // = getCurrentSimTimeNano();
    int64_t received_time; // = getCurrentSimTimeNano();
    int num_hops = 0;
...
public:
    ...
    inline void setNumHops(int num) {num_hops = num; return;}
    inline int getNumHops() {return num_hops;}
    ...
    bool control_drain = false;
    uint64_t sent_time_link = 0; // = getCurrentSimTimeNano();
    ...
    int router_hops = 0;
    std::vector<int> visited_routers;
    std::vector<uint64_t> intermediate_latencies;
...
public:
    ...
    std::vector<long long int> timestamp_flow_start;
    std::unordered_map<std::string,  int> size_flowcut;

    int num_port_hosts = 0;
    int end_port_intra = 0;
    int end_port_global = 0;

    int num_routers = 0;
    int num_groups = 0;
    int num_routers_per_group = 0;

    int global_per_router = 0;
    int intra_per_router = 0;

    // Ack Mode, true is active
    bool ackMode = false;

    int flowcut_mode = 0;
    int flowcut_value = 100000;
    float flowcut_max_ratio = 1;
    float exponential_alpha = 0;
...
public:
    ...
    virtual void initVCs(int vns, int* vcs_per_vn, internal_router_event** vc_heads, int* xbar_in_credits, int* output_queue_lengths, int* pending_credits, int* port_credit) = 0;
