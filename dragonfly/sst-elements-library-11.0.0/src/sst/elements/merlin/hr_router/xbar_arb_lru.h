...
public:
    ...
    ~xbar_arb_lru() {
        delete [] clocks_have_but_busy;
        delete [] clocks_have_pkt;
    }
    ...
    void setPorts(int num_ports_s, int num_vcs_s) {
        ...
        this->clocks_have_but_busy = new int[num_ports];
        this->clocks_have_pkt = new int[num_ports];
        for (int i = 0; i < num_ports; i++) {
            this->clocks_have_pkt[i] = 0;
            this->clocks_have_but_busy[i] = 0;
        }
                   ...
                   )
    {
        ...
        for ( int i = 0; i < total_entries; i++ ) {
            ...
            //printf("Port Busy %d \n", in_port_busy[port]); fflush(stdout);
            if ( in_port_busy[port] <= 0 && src_event != NULL) {
                ...
                this->clocks_have_pkt[port]++;
                // We can progress if the next port's input is not
                // busy and there are enough credits.
                //printf("Next port is %d - size is %d\n", next_port, src_event->getEncapsulatedEvent()->getSizeInBits() / 8);
                if ( out_port_busy[next_port] <= 0 &&
                    ...
                    int ratio = 1;
                    if (port < 16 || next_port < 16) {
                        in_port_busy[port] = (src_event->getFlitCount() * ratio);
                        out_port_busy[next_port] = (src_event->getFlitCount() * ratio); 
                        if (src_event->getNumHops() == 0 && src_event->inspectRequest()->size_in_bits / 8 > 100) {
                            in_port_busy[port] = (src_event->getFlitCount() * ratio * 2);
                        }
                    } else {
                        in_port_busy[port] = src_event->getFlitCount();
                        out_port_busy[next_port] = src_event->getFlitCount(); 
                    }
                    //printf("Flit count: %d\n", src_event->getFlitCount());
                    ...
                    *unsat_list = check;
                    ++unsat_list;
                    progress_vc[port] = -2;
                    if (in_port_busy[next_port] > 0) {
                        //progress_vc[port] = -3; // has data but is busy
                        this->clocks_have_but_busy[port]++;
                    }
                }
            }
            else {
                   ...
                   )
    {
        ...
        for ( int i = 0; i < total_entries; i++ ) {
            ...
            if ( in_port_busy[port] <= 0 && src_event != NULL) {
                ...
                // We can progress if the next port's input is not
                // busy and there are enough credits.
                //printf("Size %d - Src %d - Dest %d - Next port is %d - Rtr %d\n",src_event->getEncapsulatedEvent()->getSizeInBits() / 8, src_event->getSrc(), src_event->getDest(), next_port, getId()); fflush(stdout);
                if ( out_port_busy[next_port] <= 0 &&
                     ports[next_port]->spaceToSend(next_vc, src_event->getFlitCount()) ) {
                ...
                // std::cout << "putting at top of list" << std::endl;
                // next_list[unsat_index++] = check;
                //printf("in_port_busy[port] %d - src_event %d \n", in_port_busy[port], src_event);
                *unsat_list = check;
                ++unsat_list;
            }
        }
