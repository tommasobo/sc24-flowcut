...
bool LinkControl::send(SimpleNetwork::Request* req, int vn) {
    ...
    //printf("Draining Bool is %d - Nic %s \n", draining_here, getName().c_str());
    if (draining_here == true) {
        //printf("Currently Draining... - NIC %s\n", getName().c_str());
        //return false;
    }
    ...
    //if (ev->getSizeInBits() / 8 > 50)
    //    printf("Nic Send LinkControl %d - %ld\n", ev->getDest(),  getCurrentSimTimeNano()); fflush(stdout);
    SST::Firefly::FireflyNetworkEvent* event = dynamic_cast<SST::Firefly::FireflyNetworkEvent*>(req->inspectPayload());
    event->sent_time_ns = getCurrentSimTimeNano();

    out_handle.queue.push(ev);
    /*if ((waiting || draining_here )&& !have_packets) {
        output_timing->send(1,nullptr);
        waiting = false;
    }

    if (!draining_here) {
        printf("Not Stopping! - %s to %d\n", getName().c_str(), ev->getDest());
        if ((waiting) && !have_packets) {
            output_timing->send(1,nullptr);
            waiting = false;
        }
    } else {
        printf("Stopping! - %s to %d\n", getName().c_str(), ev->getDest());
    }*/
    ...
    //if (ev->getSizeInBits() / 8 > 50)
    //    printf("Nic Send LinkControl2 %d - %ld\n", ev->getDest(),  getCurrentSimTimeNano()); fflush(stdout);
    return true;
}
...
SST::Interfaces::SimpleNetwork::Request* LinkControl::recv(int vn) {
    ...
    delete event;
    int tmp_size = (ret->size_in_bits) / 8;
    //printf("Size Link Recv is %d - %d \n", tmp_size, vn); fflush(stdout);
    return ret;
}
...
void LinkControl::handle_input(Event* ev)
{
    ...
    if ( base_event->getType() == BaseRtrEvent::CREDIT ) {
        ...
        SST::Firefly::FireflyNetworkEvent* event_ff = dynamic_cast<SST::Firefly::FireflyNetworkEvent*>(event->getReq()->inspectPayload());
        //if (event->getSizeInBits() / 8 > 5)
            //printf("Nic Read LinkControl2 %d - drain %d - size %d - %ld - nexxt %d\n", event->getDest(), event_ff->is_drain_control, event->getSizeInBits() / 8, getCurrentSimTimeNano()); fflush(stdout);
        //printf("Value %d\n",draining_here); fflush(stdout);
        /*if (event_ff->is_drain_control) {
            printf("Draining Link - Paused to VN %d - Nic %s \n", vn, getName().c_str()); fflush(stdout);
            draining_here = true;
            //m_nic.draining = true;:

        }
        if (event_ff->drain_stop) {
            printf("Draining Ended\n"); fflush(stdout);
            draining_here = false;
            // Add output timing send 1
        }*/
        /* if (event->getSizeInBits() / 8 > 100) {
            //printf("Size is %d - Lat is %" PRIu64 "\n", event->getSizeInBits() / 8 ,lat);
        }*/
...
void LinkControl::handle_output(Event* ev)
{
    ...
    if ( found ) {
        ...
        SST::Firefly::FireflyNetworkEvent* event = dynamic_cast<SST::Firefly::FireflyNetworkEvent*>(send_event->getReq()->inspectPayload());
        //printf("Injection time %s@%d@%d@%d - %ld\n", getName().c_str(), send_event->getDest(), event->msg_id, event->packet_id_in_message, getCurrentSimTimeNano());
        ...
        //if (event->bufSize() > 50)
            //printf("Nic Send LinkControl2 %d - %ld\n", send_event->getDest(), getCurrentSimTimeNano()); fflush(stdout);
        ...
        if (sendFunctor != nullptr ) {
            //printf("Logical VN is %d\n", send_event->getLogicalVN()); fflush(stdout);
