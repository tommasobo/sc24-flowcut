...
public:
    ...
    int arbitrate(Cycle_t UNUSED(cycle), PortInterface::port_queue_t* out_q, int* port_out_credits, bool isHostPort, bool& have_packets) {
        ...
        for ( int i = 0; i < num_vcs; ++i ) {
            int vc = arb->next();
            //printf("VC -> %ld\n",  out_q[vc].size()); fflush(stdout);
            if ( out_q[vc].empty() ) {
                continue;
            } 
            have_packets = true;
            send_event = out_q[vc].front();
            if ( port_out_credits[isHostPort ? send_event->getVN() : vc] < send_event->getFlitCount() ) {
                //printf("CCredits -> %d - Flit %d\n",  port_out_credits[isHostPort ? send_event->getVN() : vc], send_event->getFlitCount()); fflush(stdout);
                continue;
            }
            vc_to_send = vc;
            found = true;
