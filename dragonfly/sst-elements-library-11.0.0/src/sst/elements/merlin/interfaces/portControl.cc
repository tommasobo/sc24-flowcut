...
bool isAckP(internal_router_event* ev) {
    return ev->is_ack;
}
...
PortControl::send(internal_router_event* ev, int vc)
{
    ...
    if ( oql_track_port ) {
        ...
        for ( int i = 0; i < num_vcs; ++i ) {
            ...
            //printf("Router %d - Adding %d credits\n", rtr_id, flits);
        }
    }
    else {
        output_queue_lengths[vc] += ev->getFlitCount();
        //printf("Router %d - Adding %d credits\n", rtr_id, ev->getFlitCount());
    }
	ev->setVC(vc);
...
void
PortControl::initVCs(int vns, int* vcs_per_vn, internal_router_event** vc_heads_in, int* xbar_in_credits_in, int* output_queue_lengths_in, int* pending_credits_in, int* port_credits)
{
...
PortControl::~PortControl() {
    ...
    if ( ack_buf != NULL ) delete [] ack_buf;
    ...
    //if ( my_port_out != NULL ) delete [] my_port_out;
...
PortControl::handle_input_n2r(Event* ev)
{
        ...
        if ( oql_track_remote ) {
            if ( oql_track_port ) {
                for ( int i = 0; i < num_vcs; ++i ) {
                    ...
                    //printf("Router %d - Removing %d credits\n", rtr_id, ce->credits);
                }
            }
            else {
	...
	case BaseRtrEvent::PACKET:
	{
        ...
        SST::Firefly::FireflyNetworkEvent* ffff = dynamic_cast<SST::Firefly::FireflyNetworkEvent*>(rtr_event->inspectRequest()->inspectPayload());
        ffff->ingress_switch_sent_alt = getCurrentSimTimeNano();
        ...
        if (rtr_event->inspectRequest()->size_in_bits > 500) {
            SST::Firefly::FireflyNetworkEvent* ffevent = dynamic_cast<SST::Firefly::FireflyNetworkEvent*>(rtr_event->inspectRequest()->inspectPayload());
            //printf("Recv in Rtr %d - %d@%d@%d@%d - Time %ld\n", rtr_id, rtr_event->getSrc(), rtr_event->getDest(), ffevent->msg_id, ffevent->packet_id_in_message, getCurrentSimTimeNano()); fflush(stdout);
        }
...
PortControl::handle_input_r2r(Event* ev)
{
	...
	case BaseRtrEvent::INTERNAL:
    {
        ...
        if (rtr_id >= 8 && event->getEncapsulatedEvent()->getSizeInBits() > 500) {
            SST::Firefly::FireflyNetworkEvent* ffevent = dynamic_cast<SST::Firefly::FireflyNetworkEvent*>(event->inspectRequest()->inspectPayload());
            //printf("Recv in Rtr %d - %d@%d@%d@%d - Time %ld - Port %d - Credits %d (InBuC %d) - Queue %d\n", rtr_id, event->getSrc(), event->getDest(), ffevent->msg_id, ffevent->packet_id_in_message, getCurrentSimTimeNano(), port_number,  xbar_in_credits[event->getVC()], port_out_credits[event->getNextPort() * num_vcs + event->getVC()], output_queue_lengths[event->getNextPort() * num_vcs + event->getVC()]); fflush(stdout);
        }
...
PortControl::handle_output(Event* ev) {
    ...
    if (topo->ackMode) {
        ...
        if ( ! ack_buf[0].empty() ) {
            ...
            internal_router_event* send_event = ack_buf[0].front();
            ack_buf[0].pop();
            ...
            xbar_in_credits[0] += size;
            if ( !oql_track_remote ) {
                if ( oql_track_port ) {
                    for ( int i = 0; i < num_vcs; ++i ) {
                        output_queue_lengths[i] -= size;
                        //printf("Router %d - Removing ACK %d credits\n", rtr_id, size);
                    }
                }
                else {
                    output_queue_lengths[0] -= size;
                    //printf("Router %d - Removing ACK %d credits\n", rtr_id, size);
                }
            }
