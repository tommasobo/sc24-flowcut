...
int UecSrc::freq = 1;
std::string UecSrc::custom_routing = "ecmp";
simtime_picosec UecSrc::flowlet_switching_value = 0;
double UecSrc::flowcut_rtt_ratio = 2;
...
int UecSrc::choose_route() {
    ...
    case ECMP_RANDOM_ECN: {
        ...
        if (custom_routing == "ecmp") {
            ...
            if (eventlist().now() - last_flowlet > flowlet_switching_value) {
                _crt_path = (random() * 1) % _paths.size();
                last_flowlet_choice = _crt_path;
                printf("Choosing new Flowlet %d at %lu\n", from, eventlist().now() / 1000);
            } else {
                _crt_path = last_flowlet_choice;
            }
        } else if (custom_routing == "flowcut") {
            if (exp_avg_rtt_flowcut > flowcut_rtt_ratio * _base_rtt) {
                _crt_path = (random() * 1) % _paths.size();
                last_flowlet_choice = _crt_path;
                printf("Choosing new Flowcut %d at %lu\n", from, eventlist().now() / 1000);
            } else {
                _crt_path = last_flowlet_choice;
            }
        }
