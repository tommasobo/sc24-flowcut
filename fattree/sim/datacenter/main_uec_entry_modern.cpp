...
int main(int argc, char **argv) {
    ...
    while (i < argc) {
        if (!strcmp(argv[i], "-o")) {
            ...
            UecSrc::set_flowcut_rtt_ratio(rtt_ratio);
            i++;
        } else if (!strcmp(argv[i], "-flowlet_value")) {
            uint64_t flowlet_gapp = atoi(argv[i + 1]);
            UecSrc::set_flowlet_gap(flowlet_gapp);
            i++;
            ...
            if (!strcmp(argv[i + 1], "ecmp")) {
                UecSrc::set_sc_routing("ecmp");
            } else if (!strcmp(argv[i + 1], "flowlet")) {
                UecSrc::set_sc_routing("flowlet");
            } else if (!strcmp(argv[i + 1], "flowcut")) {
                UecSrc::set_sc_routing("flowcut");
            } else {
                printf("Error");
                exit(0);
            }
