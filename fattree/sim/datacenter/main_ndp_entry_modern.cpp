...
int main(int argc, char **argv) {
    ...
    while (i < argc) {
        if (!strcmp(argv[i], "-o")) {
            ...
            NdpSrc::set_flowcut_rtt_ratio(rtt_ratio);
            i++;
        } else if (!strcmp(argv[i], "-alpha_flowcut")) {
            alpha_flowcut = std::stod(argv[i + 1]);
            NdpSrc::set_flowlet_alpha(alpha_flowcut);
            i++;
        } else if (!strcmp(argv[i], "-flowlet_value")) {
            uint64_t flowlet_gapp = atoi(argv[i + 1]);
            NdpSrc::set_flowlet_gap(flowlet_gapp);
            i++;
        } else if (!strcmp(argv[i], "-failure_link")) {
            int failure_l = atoi(argv[i + 1]);
            FatTreeInterDCTopology::set_num_failures(failure_l);
            i++;
            ...
            seed = atoi(argv[i + 1]);
            i++;
        } else if (!strcmp(argv[i], "-routing_sc")) {
            if (!strcmp(argv[i + 1], "ecmp")) {
                NdpSrc::set_sc_routing("ecmp");
            } else if (!strcmp(argv[i + 1], "flowlet")) {
                NdpSrc::set_sc_routing("flowlet");
            } else if (!strcmp(argv[i + 1], "flowcut")) {
                NdpSrc::set_sc_routing("flowcut");
            } else if (!strcmp(argv[i + 1], "random")) {
                NdpSrc::set_sc_routing("random");
            } else {
                printf("Error");
                exit(0);
            }
    ...
    if (tm_file != NULL) {
        ...
        for (std::size_t i = 0; i < ndp_sinks.size(); ++i) {
            delete ndp_sinks[i];
        }
