...
int FatTreeInterDCTopology::link_failed = 0;
        ...
        for (uint32_t tor = 0; tor < NTOR; tor++) {
            ...
            for (uint32_t agg = agg_min; agg <= agg_max; agg++) {
                for (uint32_t b = 0; b < _bundlesize[AGG_TIER]; b++) {
                    ...
                    if (link_failed != 0 && count_queues % link_failed == 0) {
                        printf("Failing link\n");
                        queues_nup_nlp[i][agg][tor][b] = alloc_queue(queueLogger, _downlink_speeds[AGG_TIER] / 10,
                                                                     _queue_down[AGG_TIER], DOWNLINK, AGG_TIER, false);
                    } else {
