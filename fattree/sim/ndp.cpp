...
std::string NdpSrc::custom_routing = "ecmp";
simtime_picosec NdpSrc::flowlet_switching_value = 0;
double NdpSrc::flowcut_rtt_ratio = 2;
double NdpSrc::exp_avg_alpha = 0.12;
...
void NdpSrc::processAck(const NdpAck &ack) {
    ...
    */
    ...
    exp_avg_rtt_flowcut = exp_avg_alpha * m + (1 - exp_avg_alpha) * exp_avg_rtt_flowcut;
    ...
    if ("flowcut" == custom_routing) {
        ...
        if (_flight_size == 0 && stop_sending) {
            ...
            first_flowcut = true;
            pull = true;
            total_drain_time += eventlist().now() - drain_start;
        }
    }
...
int NdpSrc::choose_route() {
    ...
    case ECMP_RANDOM_ECN: {
        ...
        if (custom_routing == "ecmp") {
            /* printf("Using ECMP"); */
            _crt_path = from;
        } else if (custom_routing == "random") {
            /* printf("Using Random"); */
            ...
            } else if (eventlist().now() - last_pkt_send_time > flowlet_switching_value) {
                _crt_path = (random() * 1) % _paths.size();
                last_flowlet_choice = _crt_path;
                last_flowlet = eventlist().now();
                /* printf("Choosing new Flowlet %d at %lu - Flowlet Value %lun", from, eventlist().now() / 1000,
                       flowlet_switching_value); */
            } else {
                /* printf("Recycling Flowlet %d at %lu - Flowlet Value %lu\n", from, eventlist().now() / 1000,
                       flowlet_switching_value); */
                _crt_path = last_flowlet_choice;
            }
        } else if (custom_routing == "flowcut") {
            if (!received_once) {
                _crt_path = from;
                last_flowlet_choice = _crt_path;
                ...
                _crt_path = last_flowlet_choice;
                /* printf("Recycling Flowcut %d at %lu - Flowcut %f vs %f -- Alpha %f\n", from, eventlist().now() /
                   1000, exp_avg_rtt_flowcut / 1000, flowcut_rtt_ratio * _base_rtt / 1000, exp_avg_alpha); */
            }
        }
        break;
    }
...
NdpSink::~NdpSink() { printf("Packets Seen %d - Packets OOO %d\n", total_seen, ooo_number); }
...
void NdpSink::receivePacket(Packet &pkt) {
    ...
    if (seqno == _cumulative_ack + 1) { // it's the next expected seq no
        ...
        ooo_number++;
        /* printf("Received OOO %d %d at time %lu\n", from, p_id, GLOBAL_TIME / 1000); */
        if (_received.empty()) {
...
void NdpSink::update_path_history(const NdpPacket &p) {
    ...
    assert(p.path_id() >= 0 && p.path_id() < 100000);
